                ...
                <div class="search-tab-header">
                  <div class="search-tab-list">
                    <ul class="ks-switchable-nav">
                      <li v-for="(item,index) in searchTypeList" :key="index" class="search-tab-item"
                        :class="{'selected':index == searchTypeIndex}" @click="selectSearchType(index)">
                        {{item.searchTypeName}}
                      </li>
                    </ul>
      ...
      <div class="screen-outer">
        <div class="main">
          <div class="main-inner">
            <div class="category" @mouseout="outCateMenu">
              <ul class="category-bd">
                <li class="cate-menu-item" :class="{'cate-menu-item-on':cateIndex==index}"
                  v-for="(item,index) in category" :key="index" @mouseover="selectCateMenu(index)">{{item.name}}</li>
              </ul>
              <div class="cate-pop" v-show="cateIndex >= 0">
                <div class="cate-part" v-for="(item1,index1) in category" :key="index1" v-show="cateIndex==index1">
                  <div class="cate-detail">
                    <dl class="cate-detail-item" v-for="(item2,index2) in item1.children" :key="index2">
                      <dt class="cate-detail-tit">
                        <a href="#" target="_blank">{{item2.name}}</a>
                        <i class="iconfont icon-arrow-right"></i>
                      </dt>
                      <dd class="cate-detail-con">
                        <a href="#" target="_blank" class="cate-detail-con-lk" v-for="(item3,index3) in item2.children"
                          :key="index3">{{item3.name}}</a>
                      </dd>
                    </dl>
                  </div>
                </div>
          ...
          <div class="member">
            <div class="member-bd">
              <div class="avatar-wrapper">
                <a href="#" class="member-home">
                  <img class="member-avatar" v-if="!loginInfo.header && loginInfo.header != ''" :src="loginInfo.header">
                  <img class="member-avatar" src="../assets/images/default_user.png" v-else></a>
              </div>
              <a class="member-nickurl" href="#">
                <span class="member-nick-info">
                  Hi
                  <strong class="member-nick">
                    {{!loginInfo?'你好':!loginInfo.nickname|| loginInfo.nickname==''?loginInfo.username:loginInfo.nickname}}
                  </strong>
                </span>
              </a>
            </div>
            <div class="member-ft">
              <!-- 未登录-->
              <div class="member-logout" v-if="!loginInfo || !loginInfo.id">
                <a href="#" class="member-logout-op">登录</a>
                <a href="#" class="member-logout-op">注册</a>
                <a href="#" class="member-logout-op">开店</a>
              </div>
              <!-- 已登录-->
              <div class="member-login" v-else>
                <div class="order-status-info">
                  <a href="#"><strong>{{orderStatusInfo.cartNum}}</strong>购物车</a><a
                    href="#"><strong>{{orderStatusInfo.waitReceiveNum}}</strong>待收货</a><a
                    href="#"><strong>{{orderStatusInfo.waitDeliverNum}}</strong>待发货</a><a
                    href="#"><strong>{{orderStatusInfo.waitPayNum}}</strong>待付款</a><a
                    href="#"><strong>{{orderStatusInfo.waitCommentNum}}</strong>待评论</a>
                </div>
              </div>
            </div>
          </div>
